...
export function createGitHubProvider(accessToken: string): GitHubProvider {
  ...
  return {
    ...
    async searchIssues(profile: SkillProfile) {
      const topLanguages = profile.languages.slice(0, 3).map((language) => language.name);
      const seenRepos = new Map<number, Repository>();
      const issues: Issue[] = [];

      for (const language of topLanguages) {
        const response = await octokit.search.issuesAndPullRequests({
          q: `is:issue is:open label:"good first issue" language:${language} no:assignee`,
          per_page: 10
        });
      ...
      const repos = sortRepositoriesByFinalScore([...seenRepos.values()]);
